...
exports.sendTestPush = exports.onAccountMembersChanged = exports.onJoinRequestRePending = exports.onJoinRequestApproved = exports.onGlobalJoinRequestCreated = exports.onJoinRequestCreated = void 0;
const functions = require("firebase-functions/v1");
const admin = require("firebase-admin");
admin.initializeApp();
const db = admin.firestore();
// Helper: determine which user(s) should be notified as the account owner/admin.
async function resolveOwnerUids(accountId) {
...
exports.onJoinRequestCreated = functions.firestore
    ...
    if (ownerCandidates.length) {
        try {
            await Promise.all(ownerCandidates.map((uid) => notifyUserTokens(uid, 'Join request received', `${requesterName || requesterEmail} requested to join ${accountId}.`, { accountId, type: 'join_request', requestUid: req.uid || '' })));
        }
        catch (e) {
...
exports.onJoinRequestRePending = functions.firestore
    .document('accounts/{accountId}/joinRequests/{uid}')
    .onUpdate(async (change, ctx) => {
    const before = change.before.data();
    const after = change.after.data();
    ...
    if (afterStatus !== 'pending')
        return; // only act when it becomes pending
    if (beforeStatus === 'pending')
        return; // ignore no-op
    const accountId = ctx.params.accountId;
    const accountDoc = await db.doc(`accounts/${accountId}`).get();
    const ownerEmail = accountDoc.get('createdByEmail') || '';
    const ownerUid = accountDoc.get('createdBy') || '';
    const requesterEmail = after?.email || 'a user';
    const requesterName = after?.displayName || '';
    if (ownerEmail) {
        await db.collection('mail').add({
            to: ownerEmail,
            message: {
    ...
    if (ownerUid) {
        await notifyUserTokens(ownerUid, 'Join request re-submitted', `${requesterName || requesterEmail} requested to join ${accountId} again.`, { accountId });
    }
});
// Helper: send FCM to all tokens for a user doc id (users/<uid>/tokens/*)
async function notifyUserTokens(uid, title, body, data) {
...
exports.onAccountMembersChanged = functions.firestore
    ...
    if (!change.before.exists && change.after.exists) {
        ...
        if (creatorUid) {
            await notifyUserTokens(creatorUid, 'Account created', `Your account ${accountId} was created successfully.`, { accountId });
        }
        return;
    }
    ...
    await Promise.all(added.map(async (uid) => {
        await notifyUserTokens(uid, 'Added to account', `You were added to account ${accountId}.`, { accountId });
    }));
    // Optionally notify owner/creator that members were added
    const creatorUid = after.createdBy || '';
    if (creatorUid) {
        await notifyUserTokens(creatorUid, 'Member added', `New member(s) added to ${accountId}: ${added.join(', ')}`, { accountId });
    }
});
...
exports.sendTestPush = functions.https.onCall(async (data, context) => {
    ...
    const body = data?.body || 'This is a test push from BudgetBuddy.';
